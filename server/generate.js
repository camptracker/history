...
async function generateBooks(date) {
  const data = await fetchJSON('https://www.googleapis.com/books/v1/volumes?q=subject:self-help&orderBy=newest&maxResults=5');
  const books = (data.items || []).slice(0, 2);
  return books.map(b => {
    ...
    return {
      ...
      summary: info.subtitle || (info.description || '').slice(0, 200),
